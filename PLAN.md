...
- Страница `/sessions`
- Секция сессий на странице спота
...
**Backend:**
- [x] Модель `Rental` (name, description, lat, lon, city, items[], prices, contacts, media)
- [x] CRUD: `POST/GET/PUT/DELETE /rentals`, `GET /rentals/my`
- [x] Список с фильтром по городу, типу инвентаря, радиусу
- [ ] Привязка к споту (опционально): `rental_id` на споте

**Frontend:**
- [x] Страница `/rentals` — список точек проката
- [x] Карточка проката: фото, инвентарь, цены, контакты
- [x] Форма добавления и редактирования
- [x] Фильтр по типу инвентаря (в API, на фронте — чипы выбора)
- [ ] Карта на странице проката
...
- Привязка к споту — "рядом есть прокат" (TODO)
