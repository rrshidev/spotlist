...
export default function SpotPageClient() {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: spot?.name || 'SpotList', url });
        return;
    ...
    try {
      ...
      setCopied(true);
      addToast(t('spotDetail.linkCopied'), 'success');
      setTimeout(() => setCopied(false), 2000);
    } catch {
      addToast(t('spotDetail.shareError'), 'error');
    }
  };
    ...
    <div className="flex-1 p-4 pb-20">
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center gap-2 mb-4">
          ...
          <button
            onClick={handleShare}
            className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[#0a0a0f] border border-[#1f1f2e] text-white/60 hover:text-white hover:border-[#39ff14] transition-colors"
            title={t('spotDetail.share')}
          >
            {copied ? <Check className="w-4 h-4 text-[#39ff14]" /> : <Share2 className="w-4 h-4" />}
            <span className="text-sm">{copied ? t('spotDetail.copied') : t('spotDetail.share')}</span>
          </button>
          <button
            onClick={() => window.open(`https://t.me/share/url?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(spot?.name || 'SpotList')}`, '_blank', 'noopener')}
            className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[#0a0a0f] border border-[#1f1f2e] text-white/60 hover:text-white hover:border-[#0088cc] transition-colors"
            title={t('spotDetail.shareTelegram')}
          >
            ...
            <span className="text-sm">{t('spotDetail.shareTelegram')}</span>
          </button>
        </div>
